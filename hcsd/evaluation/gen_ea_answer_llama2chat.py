...
def get_model_answers(
    ...
    model = EaModel.from_pretrained(
        ...
        partial_offload_SD=partial_offload_SD,
        total_token=args.total_token,
        depth=args.depth,
        top_k=args.top_k,
    ...
    for _ in range(3):
        ...
        for j in range(len(question["turns"])):
            ...
            if partial_offload_SD:
                input = torch.as_tensor(input_ids).cuda()
            else:
                input = torch.as_tensor(input_ids, device=torch.device("cpu"))
    ...
    for question in tqdm(questions):
        ...
        print(f"Max memory used after question {question['question_id']}: {max_memory / 1024**2} MB")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=-1,
        help="The number of layers offloaded to the GPU.",
    )

    parser.add_argument(
        "--partial-offload",
        action="store_true",
        help="Enable partial offload",
    )
    ...
    parser.add_argument(
        "--draft-model-type",
        type=str,
        help="The draft model  tinyllama1.1B-> tinyllama  shearedllama1.3B -> shearedllama   eagle-2 -> eagle  HCSD-> hcsd",
    )
    parser.add_argument(
    ...
    run_eval(
        ...
        args.ngl,
        args.partial_offload,
        args.partial_offload_SD,
        args.model_id,
        question_file,
        args.question_begin,
        args.question_end,
