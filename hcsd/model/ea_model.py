...
class EaModel(nn.Module):
    ...
    def __init__(
        ...
        # ##########
        # self.ea_layer.to(dtype=self.base_model.dtype, device='cpu')
    ...
    def from_pretrained(
        ...
        if Type=='LlamaForCausalLM':
            # base_model_1 = KVLlamaForCausalLM.from_pretrained(
            #     base_model_path, **kwargs
            # )
            # print(base_model_1)

            # base_model_2 = torch.quantization.quantize_dynamic(
            #     base_model_1, {torch.nn.Linear}, dtype=torch.qint8 ,inplace=True  # 量化 Linear 层
            # )
            # print(base_model_2)
            # torch.save(base_model_2, "quantized_model_12.1_2.pth")
            base_model = torch.load("quantized_model_12.1_2.pth", map_location="cpu")
            print(base_model)
